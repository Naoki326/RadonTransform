...
namespace MyAlg
{
	...
	void RadonLines::FeedData(const Mat& image)
	{
		...
		//利用傅里叶切片定理实现Radon变换
		//执行二维傅里叶变换
		shared_ptr<double> p_origin(new double[padedw_ * padedh_], std::default_delete<double[]>());//输入
		...
		//执行一维傅里叶逆变换
		shared_ptr<double> p_polar_real(new double[padedw_], std::default_delete<double[]>());//输入
		shared_ptr<double> p_polar_imag(new double[padedw_], std::default_delete<double[]>());
		shared_ptr<double> it_amp(new double[padedw_], std::default_delete<double[]>());//输出
